# John Bigelow Papers (Bigelow Collection)

## Overview
The **John Bigelow Papers** consist of the extensive correspondence of Bigelow and his family, his scrapbooks and writings, records and correspondence detailing his professional activities, diaries and journals belonging to Bigelow and other family members, genealogical documents and records of the Bigelow family, and a variety of photographs.

Funding for the digitization of this collection was provided by the **National Historical Publications & Records Commission (NHPRC)**.

---

## Source & Provenance
- **Institution:** Schaffer Library, Union College  
- **Collection Name:** John Bigelow Papers  
- **Type of Resource:** Collection  
- **Source System:** Arches  
- **Persistent URL:** [https://arches.union.edu/bigelow](https://arches.union.edu/bigelow)  
- **Digital Publisher:** Union College  
- **Finding Aid:** [Link to ArchivesSpace Record](#)  
- **Rights Statement:** No Copyright – United States  

---

## Data Files
- `8-68f6550c847ac.csv` — CSV export of collection metadata (cleaned to remove empty columns).  
  - Delimiter: `,`  
  - Encoding: UTF-8  
  - Header row included: Yes  
  - Line endings: LF/CRLF (platform-dependent)  

---

## Controlled Access Terms

### Subject Headings
- Union College (Schenectady, N.Y.) — Students — Societies, etc  
- Sigma Phi  
- New York State — History — Civil War, 1861–1865  
- Philomathean Society (Union College)  
- New York (State) — Social life and customs  
- New York (State) — History — 1775–1865  

### Personal Names
- Bigelow, John, 1817–1911  
- Bigelow, Jane Tunis Poultney  
- Conrad, Bryan  

### Corporate Names
- Union College (Schenectady, N.Y.)  

### Geographic Names
- Panama Canal (Panama)  

---

## Field Notes
The CSV contains metadata for individual records or objects in the Bigelow Papers. Common fields include:
- `title` — Item title or formal name  
- `creator` — Person or entity responsible  
- `date` — Normalized or textual date  
- `identifier` — Local system identifier or call number  
- `subjects_*` — Controlled vocabularies or topical terms  
- `rights` — Rights statement (No Copyright – United States)  

---

## Cleaning Steps
- Removed all columns that were completely empty across all rows  
- Dropped redundant or invariant metadata fields (e.g., internal system fields)  
- Verified UTF-8 encoding and standardized field names  

### Reproducibility
```python
import pandas as pd

df = pd.read_csv("8-68f6550c847ac.csv")
df = df.dropna(axis=1, how="all")
df.to_csv("8-68f6550c847ac_clean.csv", index=False)
